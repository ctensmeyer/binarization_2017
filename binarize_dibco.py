...
NET_FILE = os.path.join(os.path.dirname(__file__), "dibco_model.prototxt")
WEIGHTS_DIR = os.path.join(os.path.dirname(__file__), "weights_dibco")
...
def main(in_image, out_image):
	...
	data = np.concatenate([image[:,:,np.newaxis], rd_im], axis=2)
